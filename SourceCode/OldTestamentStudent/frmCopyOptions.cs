...
namespace OldTestamentStudent
{
    public partial class frmCopyOptions : Form
    {
         ...
         *   versionCode  Alternative to whichVersion   MT = 0              LXX = 16         *
         *                                                                                   *
         *-----------------------------------------------------------------------------------*/
        int destCode, refCode, accentCode, rememberCode, versionCode;
        ...
        public int VersionCode { get => versionCode; set => versionCode = value; }
        ...
        private void setupCodes()
        {
            ...
            switch (whichVersion)
            {
                case 1: versionCode = 0; break; //MT
                case 2: versionCode = 16; break; //LXX
                default: versionCode = -1; break;
            }
        }
        ...
        public void performCopy()
        {
            ...
            globalVars.setCopyOption((typeOfCopy - 1) * 4 + versionCode, destCode);
            globalVars.setCopyOption((typeOfCopy - 1) * 4 + 1 + versionCode, refCode);
            globalVars.setCopyOption((typeOfCopy - 1) * 4 + 2 + versionCode, accentCode);
            globalVars.setCopyOption((typeOfCopy - 1) * 4 + 3 + versionCode, rememberCode);
            ...
            switch (accentCode)
            {
                ...
                case 2:
                    ...
                    for( idx = 0; idx < noOfWords; idx++)
                    {
                        switch( whichVersion )
                        {
                            case 1: tempText = hebrewMethods.removeAccents(brokenText[idx]); break;
                            case 2: tempText = greekMethods.reduceToBareGreek(brokenText[idx], true); break;
                        }
