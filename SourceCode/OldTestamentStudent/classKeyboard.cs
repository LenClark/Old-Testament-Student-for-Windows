...
//                gbTextDestination.Controls.Add(rbtnTemp);
        ...
        private void hebKeyboard_button_click(object sender, EventArgs e)
        {
            ...
            if (globalVars.RbtnMTDestination[0].Checked) updateNotes(keyVal, clickedTag, (RichTextBox)globalVars.getGroupedControl(globalVars.RichtextBoxCode, 2));
            if (globalVars.RbtnMTDestination[1].Checked) updateSearchBox(keyVal, clickedTag, (TextBox) globalVars.getGroupedControl( globalVars.TextboxCode, 0), false);
            if (globalVars.RbtnMTDestination[2].Checked) updateSearchBox(keyVal, clickedTag, (TextBox)globalVars.getGroupedControl(globalVars.TextboxCode, 1), true);
        }
...
//            int[] rbtnLeft = { 15, 90, 224, 300 };
        ...
        private void updateGkNotes( String keyVal, int clickCode, bool isModifyingChar, bool isNormalChar, RichTextBox rtxtNotes)
        {
            int currPstn;
        ...
        private void updateNotes(String unicodeRepresentation, int tagValue, RichTextBox rtxtNotes)
        {
            ...
            else
            {
                switch (tagValue)
                {
                    case 13:
                    case 26:
                        deleteRtxt(rtxtNotes, fullNote, currPstn);
                        break;
                    case 39: break; // do nothing
                }
            }
        }
        ...
        private void updateSearchBox(String unicodeRepresentation, int tagValue, TextBox txtTarget, bool isSecondary)
        {
            ...
            else
            {
                switch (tagValue)
                {
                    case 13:
                    case 26:
                        if (txtTarget.Text.Length > 0) txtTarget.Text = txtTarget.Text.Substring(0, txtTarget.Text.Length - 1);
                        break;
                    case 39: txtTarget.Text = ""; break;
                }
            }
        }
